...
// @Produce json
// @Param id path int true "Keyword ID"
// @Param keyword body dto.UpdateKeywordRequest true "Keyword update data"
// @Success 200 {object} dto.KeywordResponse
// @Failure 400 {object} dto.ErrorResponse
// @Failure 404 {object} dto.ErrorResponse
// @Failure 500 {object} dto.ErrorResponse
// @Router /api/keywords/{id} [put]
func (h *KeywordHandler) UpdateKeyword(c *gin.Context) {
	...
	var req dto.UpdateKeywordRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		c.JSON(http.StatusBadRequest, dto.ErrorResponse{
			Error:   "validation_error",
			Message: err.Error(),
	...
	keyword, err := h.keywordUseCase.UpdateKeyword(id, req.GroupID)
	if err != nil {
		if usecases.IsDomainError(err) {
			code := usecases.GetDomainErrorCode(err)
			status := http.StatusInternalServerError
			...
			case usecases.ErrorKeywordNotFound:
				status = http.StatusNotFound
			case usecases.ErrorKeywordUpdate:
				status = http.StatusInternalServerError
			}
	...
	c.JSON(http.StatusOK, dto.KeywordResponse{
		ID:      keyword.ID,
		Value:   keyword.Value,
		SiteID:  keyword.SiteID,
		GroupID: keyword.GroupID,
